	<nav class="ts-sidebar">
		<ul class="ts-sidebar-menu">
			<li class="ts-label">ADMINISTRATOR</li>
			<li><a href="dashboard.php"><i class="fa fa-dashboard"></i> Dashboard</a></li>
		
			<li><a href="#"><i class="fa fa-users"></i> User</a>
				<ul>
					<li><a href="add-user.php">Add User</a></li>
					<li><a href="manage-user.php">Manage User List</a></li>
				</ul>
			</li>

			<li><a href="#"><i class="fa fa-files-o"></i> Brands</a>
				<ul>
					<li><a href="add-brand.php">Add Brand</a></li>
					<li><a href="manage-brands.php">Manage Brand List</a></li>
				</ul>
			</li>

			<li><a href="#"><i class="fa fa-sitemap"></i> Vehicles</a>
				<ul>
					<li><a href="add-vehicle.php">Add Vehicle</a></li>
					<li><a href="manage-vehicles.php">Manage Vehicle List</a></li>
				</ul>
			</li>
			
			<li><a href="manage-bookings.php"><i class="fa fa-users"></i> Manage Booking</a></li>
			<li><a href="manage-conactusquery.php"><i class="fa fa-desktop""></i> Manage Conatctus Query</a></li>
		</ul>
	</nav>